...
@ray.remote(num_cpus=5, max_concurrency=20)
class FullyAsyncRollouter(RayPPOTrainer):
    ...
    def __init__(
        ...
        # 初始化异步队列
        self.pending_queue = asyncio.Queue(maxsize=1000)
    ...
    async def set_parameter_server(self, param_server):
    ...
    async def _feed_samples(self):
        ...
        print("fedd samples feed samples")
        sample_count = 0
        should_stop = False
        ...
        for epoch, batch_dict in continuous_iterator:
            ...
            for rollout_n_index in range(self.config.actor_rollout_ref.rollout.n):
                ...
                # print(self.pending_queue.qsize())
                # 检查是否到达最后一步
    ...
    async def _process_single_sample_streaming(self, partial_rollout_sample):
        ...
        # 调用异步生成方法
        try:
            agent_loop_output, processing_time = await self.async_rollout_manager.generate_sample_async(
                partial_rollout_sample.full_batch, partial_rollout_sample.sample_id
            )
        except Exception as e:
            print(f"[FullyAsyncRollouter] 处理样本时发生错误: {e}")
            # 处理错误，可能需要重新提交任务或记录日志
            raise
    ...
    async def _consumer_worker(self):
        ...
        while True:
            ...
            #     print(f"[DEBUG] result_queue empty:{self.result_queue.qsize()}")
            #     await asyncio.sleep(0.1)
            #     continue
            # print(f"[DEBUG] 已放入 result_queue:{self.result_queue.qsize()}")
            rollout_sample = await self.result_queue.get()
            # print(f"[DEBUG] 已取出 result_queue:{self.result_queue.qsize()}")

            # 直接将 RolloutSample 放入消息队列
            try:
                success = await self.message_queue_client.put_sample(
                    sample_data=ray.cloudpickle.dumps(rollout_sample),
    ...
    async def _streaming_generation_main(self):
        ...
        try:
            ...
            # 等待任务结束
            await asyncio.gather(self.processor_task, self.consumer_task, return_exceptions=False)
    ...
    async def fit(self):
        ...
        if self.param_server is None:
            raise ValueError("param_server client not set. Call set_param_server() first.")
        ...
        try:
            # 并发运行生成和监控任务
            await asyncio.gather(generation_task, monitor_task, return_exceptions=False)
        except Exception as e:
    ...
    async def _should_pause_generation(self) -> bool:
        ...
        current_trainer_version = queue_stats["param_version"]

        version_diff = self.current_param_version - current_trainer_version
        # print(queue_size, current_trainer_version, version_diff)
